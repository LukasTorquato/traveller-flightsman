...
- runtime/history/observations.jsonl (30 v2 rows + 1 combined run_metadata row)
- runtime/reports/<run_date>.md (combined report)
- runtime/state/rotation.json (advanced cursors)

Usage:
  python3 scripts/write_synthetic_scan_outputs.py \
    --allow-synthetic-runtime-write <run_date> <started_at_iso> <ended_at_iso>
"""
import json
import pathlib
import sys
...
_USAGE = (
    "usage: python3 scripts/write_synthetic_scan_outputs.py "
    "--allow-synthetic-runtime-write <run_date> <started_at_iso> <ended_at_iso>"
)
...
def main(argv):
    if len(argv) != 5 or argv[1] != "--allow-synthetic-runtime-write":
        print(_USAGE, file=sys.stderr)
        return 2
    if not (ROOT / "runtime" / ".git").exists():
        print(
            "error: runtime submodule is not initialized; run "
            "git submodule update --init runtime",
            file=sys.stderr,
        )
        return 1

    run_date, started, ended = argv[2:]
    ...
    report_lines += [
        ...
        "## Data transparency",
        "",
        "Pricing for this scan was generated by `scripts/build_synthetic_fares.py`, which anchors each route to its most",
        "recent v2 observation in `runtime/history/observations.jsonl` and applies small deterministic",
